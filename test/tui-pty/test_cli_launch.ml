...
let%expect_test "default launch enters the home stage" =
  ...
  print_fact "process remains interactive" (not (Term.exited t));
  [%expect
    {|
...
let run_resume_case name command expected unexpected =
  ...
  Term.run project ~env:reduced_motion ~rows:24 ~cols:80 ~command
...
let%expect_test "launch flags resolve and replay the requested session" =
  ...
  run_resume_case "cli-session" [ "--session"; "ses_older" ] "the older prompt"
    "the newer prompt";
...
let%expect_test "review launch opens directly and closes the process" =
  ...
  Term.wait_exit t;
  print_fact "close exits" (Term.exited t);
  [%expect
    {|
