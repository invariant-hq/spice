...
(* The global prompt history the runtime loads at boot, written to the isolated
   config home ([XDG_CONFIG_HOME]/spice/history.jsonl) before launch. One line
...
let history project lines =
  Util.write_file
    (Project.scratch project (Filename.concat "config" "spice/history.jsonl"))
    (String.concat "\n" lines ^ "\n")
...
let sessions_contain project needle =
  let rec contains_file path =
    ...
    | exception Sys_error _ -> false
  in
  contains_file (Project.root project)
