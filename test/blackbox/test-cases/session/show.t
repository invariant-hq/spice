...
An unfinished active turn with no live owner is reported as active.
  ...
  $ mkdir -p .spice/sessions/stale
  $ cat > .spice/sessions/stale/session.json <<'JSON'
...
Pending permission requests are reported as waiting.
  ...
  $ mkdir -p .spice/sessions/blocked
  $ cat > .spice/sessions/blocked/session.json <<'JSON'
...
restore, not a continuation command the lifecycle gate would reject.
  ...
  $ mkdir -p .spice/sessions/archived-blocked
  $ cat > .spice/sessions/archived-blocked/session.json <<'JSON'
