...
not contain a session document.
  ...
  $ mkdir -p .spice/sessions/empty-dir
  $ spice session list
  ID  PHASE  AGE  TITLE

Manually seeded canonical documents are listed and can be shown/exported.

  $ mkdir -p .spice/sessions/manual
  $ printf '%s\n' "{\"version\":1,\"id\":\"manual\",\"metadata\":{\"title\":\"Manual\",\"status\":\"active\",\"cwd\":\"$PWD\",\"created_at\":1,\"updated_at\":1},\"events\":[]}" > .spice/sessions/manual/session.json
...
The requested id must match the id inside the document.
  ...
  $ mkdir -p .spice/sessions/wanted
  $ printf '%s\n' "{\"version\":1,\"id\":\"actual\",\"metadata\":{\"status\":\"active\",\"cwd\":\"$PWD\",\"created_at\":1,\"updated_at\":1},\"events\":[]}" > .spice/sessions/wanted/session.json
  ...
  $TESTCASE_ROOT/.spice/sessions/wanted/session.json
  document id actual does not match requested id wanted
  [1]

Malformed JSON fails loudly with the document path.

  $ mkdir -p .spice/sessions/bad-json
  $ printf '[' > .spice/sessions/bad-json/session.json
...
Unsupported document versions fail rather than silently migrating.
  ...
  $ mkdir -p .spice/sessions/v2
  $ printf '%s\n' "{\"version\":2,\"id\":\"v2\",\"metadata\":{\"status\":\"active\",\"cwd\":\"$PWD\",\"created_at\":1,\"updated_at\":1},\"events\":[]}" > .spice/sessions/v2/session.json
  ...
  $TESTCASE_ROOT/.spice/sessions/v2/session.json
...
compatibility repair.
  ...
  $ mkdir -p .spice/sessions/no-cwd
  $ printf '%s\n' '{"version":1,"id":"no-cwd","metadata":{"title":"Old","status":"active","created_at":1,"updated_at":1},"events":[]}' > .spice/sessions/no-cwd/session.json
...
Old lifecycle event shapes are not accepted as semantic session events.
  ...
  $ mkdir -p .spice/sessions/old-event
  $ printf '%s\n' "{\"version\":1,\"id\":\"old-event\",\"metadata\":{\"status\":\"active\",\"cwd\":\"$PWD\",\"created_at\":1,\"updated_at\":1},\"events\":[{\"type\":\"archived\"}]}" > .spice/sessions/old-event/session.json
...
Unknown document fields fail loudly.
  ...
  $ mkdir -p .spice/sessions/unknown-field
  $ printf '%s\n' "{\"version\":1,\"id\":\"unknown-field\",\"metadata\":{\"status\":\"active\",\"cwd\":\"$PWD\",\"created_at\":1,\"updated_at\":1},\"events\":[],\"extra\":true}" > .spice/sessions/unknown-field/session.json
...
structured corrupt entries.
  ...
  spice: corrupt session document at $TESTCASE_ROOT/.spice/sessions/bad-json/session.json
  spice: corrupt session document at $TESTCASE_ROOT/.spice/sessions/no-cwd/session.json
  spice: corrupt session document at $TESTCASE_ROOT/.spice/sessions/old-event/session.json
  spice: corrupt session document at $TESTCASE_ROOT/.spice/sessions/unknown-field/session.json
  spice: corrupt session document at $TESTCASE_ROOT/.spice/sessions/v2/session.json
  spice: corrupt session document at $TESTCASE_ROOT/.spice/sessions/wanted/session.json
...
mode, and fails loudly in both modes.
  ...
  {"schema_version":1,"type":"session","session":{"id":"no-cwd","phase":"error","path":"$TESTCASE_ROOT/.spice/sessions/no-cwd/session.json","message":"Missing member cwd in session metadata object"}}
  [1]
