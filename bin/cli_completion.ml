...
(* Standalone shell completion scripts: cmdliner's generic completion
   function (protocol version 1, spoken by this binary via [--__complete])
   specialized to spice, as the cmdliner tool's [tool-completion --standalone]
   would emit. Vendored from cmdliner 2.1.1; the protocol version is checked
   at runtime by the scripts, so skew fails loudly rather than silently. *)
...
if ! declare -F _spice_cmdliner > /dev/null; then
  _completion_loader _spice_cmdliner
fi
complete -F _spice_cmdliner spice|script}

let zsh_script =
  {script|#compdef spice
function _spice_cmdliner {
  local w=("${words[@]}") # Keep words intact for restart completion
...
Register-ArgumentCompleter -Native -CommandName spice -ScriptBlock $Global:_spice_cmdliner|script}
...
let command =
  let man =
    [
      ...
      `Pre "  spice completion zsh > ~/.local/share/zsh/site-functions/_spice";
      `Pre
        "  spice completion bash > \
         ~/.local/share/bash-completion/completions/spice";
    ]
  in
  CCmd.v
    (CCmd.info "completion" ~doc:"Print a shell completion script."
       ~docs:s_config_commands ~man ~exits)
    (exit_term CTerm.(const completion $ shell_arg))
