...
let rec download_response ~sw ~http ~remaining uri =
  ...
  if code >= 300 && code < 400 then
    ...
    | Some _ -> Error "too many redirects while downloading DeepSeek model"
    | None -> Error "DeepSeek model download redirect did not include Location"
  else if code >= 200 && code < 300 then Ok (response, body)
  else Error (Printf.sprintf "DeepSeek model download returned HTTP %d" code)
...
let write_body_to_file ~cancelled ?observe_download ~model ~artifact ~path
    ~total body =
        ...
        if cancelled () then Error `Cancelled
          ...
          | count ->
              ...
              if should_emit then begin
                last_emit := received;
                download_progress ~observe_download ~model ~artifact ~path
            ...
            | Some http -> (
                try
                  ...
                  download_progress ~observe_download ~model:id ~artifact ~path
                    ~received:0L ~total:(Some artifact.size)
                    ~phase:Download.Checking;
                  let* response, body =
                    match
                      download_response ~sw ~http ~remaining:8
                        (Uri.of_string artifact.url)
                    with
                  ...
                  in
                  download_progress ~observe_download ~model:id ~artifact ~path
                    ~received:0L ~total ~phase:Download.Downloading;
                  let* received, digest =
                    match
                      write_body_to_file ~cancelled ?observe_download ~model:id
                  ...
                  let part_path = path ^ ".part" in
                  download_progress ~observe_download ~model:id ~artifact ~path
                    ~received ~total:(Some artifact.size)
                    ~phase:Download.Verifying;
                  let* () =
                    match verify_file_size part_path artifact.size with
                    | Ok () -> Ok ()
                    | Error message -> startup_provider_error message
                  in
                  let* () =
                    if String.equal digest artifact.sha256 then Ok ()
                    else
                      startup_provider_error
                        (Printf.sprintf "expected SHA-256 %s, got %s"
                           artifact.sha256 digest)
                  in
                  Eio.Path.rename (fs_path env part_path) (fs_path env path);
                  download_progress ~observe_download ~model:id ~artifact ~path
                    ~received:artifact.size ~total:(Some artifact.size)
                    ~phase:Download.Installed;
                  Log.info (fun m ->
                      m "model installed model=%s path=%s" id path);
                  Ok path
                with
