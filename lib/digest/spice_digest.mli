...
(** Stable SHA-256 digests for exact byte sequences.
    ...
    input is responsible for its own domain separation; {!key} performs that
    join for derived identifiers. This module is deterministic and pure, but it
    is not an authentication boundary and does not provide keyed authentication.
*)
...
val key : length:int -> string list -> string
(** [key ~length parts] is the first [length] lowercase hexadecimal characters
    of the SHA-256 digest of [parts] joined by NUL bytes. The join gives the
    parts domain separation: distinct part lists cannot collide by
    concatenation. A single part is hashed with no separator, and [~length:64]
    yields its full digest.

    [length] is required so the collision-risk choice stays explicit at each
    call site. Short prefixes are suitable for display or local generated
    identifiers only when the caller accepts the collision risk for the chosen
    length.

    Raises [Invalid_argument] if [length] is not in the range \[[0];[64]\]. *)
    ...
    type t =
      | Invalid_identity
          (** The type for identity parse errors.
              ...
              [Invalid_identity] reports any violation of the canonical identity
              grammar. *)
