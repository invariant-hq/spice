...
(** The per-tool transcript grammar (02-tools.md).
    ...
    inline — a full diff for an {!Update}, a content preview for a {!Create},
    the live todo board. Nothing raw ever reaches the transcript: no JSON, no
...
type verb =
  ...
  | List
  | Search
      (** [search_text], [glob], and [ocaml_search_expressions] — text, path,
          and structural search all summarize as [Found …] *)
  ...
  | Definition
      (** ocaml_find_definitions — the identifier is the argument, the resolved
          location the result *)
  | References
      (** ocaml_find_references — the occurrence counts are the result, the
          first locations disclosed beneath *)
  ...
  | Wait  (** wait_subagents *)
  | Question
      (** ask_user — the question is the argument, the answer the result *)
  | Other of string
...
type detail =
  ...
  | Todos of todo_item list
      (** the live todo board (02-tools.md §Todo block) *)
...
(** [preview ~take ~cap lines] is the {!Preview} detail auto-showing at most
    ...
    failed {!Shell} tail — with the remainder folded into [overflow]. This is
    the one shared truncation law (02-tools.md §Truncation): when exactly one
    line would remain it is shown in full rather than counted, so the overflow
    never hides a single line behind [… +1 lines ▸]. *)

val header_argument : width:int -> verb:verb -> string -> string
(** [header_argument ~width ~verb argument] pre-truncates [argument] to the
    columns a [width]-wide header leaves after the dot, the [verb] label, and
    the parentheses, with a trailing [ … ] (02-tools.md §Truncation). A
...
(** [header verb ~argument ~dot] is the [⏺ Verb(argument)] row: the dot colored
    by [dot], the verb bold, the argument default foreground. A caller that
    holds the terminal width pre-truncates [argument] through {!header_argument}
    first; the one caller without a width (the shell-command running header)
    passes it raw and the argument flex-clips. *)
...
(** [view ~width t] is the whole block: {!header}, the {!result} summary line,
    and the per-tool {!detail} hanging beneath it. [width] is the terminal
    column count, threaded to the header argument and the detail rows (preview,
    todo, diff labels) so each pre-truncates rather than clipping off the edge.
*)
