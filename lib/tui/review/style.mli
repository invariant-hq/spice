...
(** The review screen's design tokens: colors, text styles, glyphs, layout
    ...
    language rather than sharing {!Spice_tui.Theme}, which is a private
    module of the [spice_tui] library and so unreachable from this separate
    library — the same reason the old [lib/tui/review] carries its own [style.ml]
    (doc/plans/tui-next-review.md §divergence 5). The constant values are kept in
    sync with tui-next's [Theme] by convention; deliberate changes travel through
    the docs to both. *)
...
(** [color_accent] is paprika, the one brand hue: cursors, selection, active
    ...
    [Theme] is a private module of [spice_tui] and unreachable here. A change
    to the brand hue must touch both this constant and [Theme.color_accent];
    that drift risk is the price of the separate library. *)
...
(** [diff_quieted] is the reviewed-scope variant: add/del backgrounds dropped and
    signs muted, so settled content stops shouting. *)
