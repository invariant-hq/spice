...
(** The wide-terminal side panel — the context & activity column
    ...
    right of a [│] rule. It carries the turn's activity while a turn streams
    (the live todo board, then the tool feed and threads as their seams land)
    and a quiet ambient glance while idle. The composer and footer span the full
    width below it.
    ...
    - {b The transcript is fluid} (Thibaut's ruling, 2026-07-08): it renders at
      ...
      {!transcript_width} with it open. There is no 80-column cap; the earlier
      cap design is dead. The pane is sized transcript-first: an open pane never
      takes the transcript below 80 columns (see {!width}).
    - {b Pane presence is a pure function of width, never of turn state.} At
      [>= min_cols] (with the {!keep_open_cols} hysteresis, {!presence}) the
      region exists; the turn varies its {e content}, not the region. A
      turn-scoped pane over a fluid transcript would rewrap the whole document
      at every turn boundary — so presence tracks width, and reflow happens only
      on a real terminal resize across the threshold.
    ...
    glance otherwise — renders it to rows through that content's own module
    (e.g. {!Todo_board.view}), and hands the rows here as [~right]. So each
    content source lives in exactly one module and the pane re-implements none
    of them, and the shell — the one source of visibility truth — routes a
    tenant's rows to exactly one region (the strip {e or} the pane, never both:
    the double-render law).

    {b Display-only.} The pane never captures keys, focus, or the wheel. The
    wheel always scrolls the transcript (01-transcript.md); pane overflow
    degrades to the tenant's own digest rather than scrolling — the pane is
    never a scrollport. *)
...
(** [keep_open_cols] is 109: once present, the region persists down to this
    width, a hysteresis dead band so dragging a resize across {!min_cols} does
    not thrash the layout. It is the lowest width that still holds the 80-column
    transcript floor beside the 28-column pane minimum and the 1-column rule (80
    \+ 28 + 1), so an open pane never dips the transcript below its floor. *)
...
(** [content_rows ~rows] is the row budget the pane grants a tenant at terminal
    height [rows] — approximately the transcript-region height, so a tenant
    folds its own content to fit (the shell passes it as {!Todo_board.view}'s
    [~max_rows]). It is an approximation; {!frame} clips with a hidden overflow
    so an over-budget tenant never grows the row and pushes the composer (the
    layout-stability law). Always [>= 1]. *)
...
(** [frame ~cols ~open_ ~left ~right] is the transcript region:
    ...
    empty and the region still shows (an over-degraded glance); it never
    collapses the pane on turn state. The [│] rule is a left border in
    {!Theme.color_rule} on the pane column, stretched to the region height by
    the row's default item alignment, so it spans the full height however short
    the rows are. The pane column clips its overflow, so it can never push the
    composer.
