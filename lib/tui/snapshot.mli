...
(** The static session-start facts, assembled once at launch.

    This is tui-next's own status projection (doc/plans/tui-next.md, "tui-next
    builds its own snapshot from host calls") — the slow facts a user reads once
    on entry: version, model, cwd, the context window, and any non-default
    permission/sandbox posture. The live facts (dune, worktree, CRs, last
    session) are the {!Home.Brief.t}; the two are kept apart because they refresh on
    different clocks (04-header-footer.md, "Header vs footer").
