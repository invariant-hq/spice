...
(** Distilling a settled tool call into its transcript block (02-tools.md).
    ...
    The pure, state-free construction half of the tool grammar: given a tool call
    that has reached its result, produce the settled {!Tool_block.t} the document
    renders. {!Tool_block} owns the block type and its views; this module owns how
    a call becomes one. Every function is a total function of its inputs — no
    clock, no filesystem, no reducer state — so replay
...
(** [argument_of_call call] is the header's primary argument, read off the call's
    typed input: the [@role — task] for a subagent spawn, the queried [path:line]
    for the OCaml navigation tools, the question for [ask_user], the generic first
    string field otherwise. Shared by the settle path and the live-tail running
    rows. *)
...
(** [denied call] is the warning-dotted [denied] stub a permission refusal settles
    to the document, so the record shows the refused call (02-tools.md §Header). *)
