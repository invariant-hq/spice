...
(** The model + effort panel (03-ia-screens-overlays.md §The three forms,
    ...
    provider, type-to-filter, an inline reasoning-effort line for the highlighted
    model, esc closes. A panel, not an overlay — it replaces the composer region
    below the [▔] boundary while the transcript stays above.
    ...
    {!view} through {!Panel.view}. The panel reads no config, catalog, clock, or
    host: every fact arrives as the {!facts} record the runtime assembles from the
    host catalog and the per-provider account phases, formatted once.
    ...
    [Spice_host.Models.for_select] and persists as [Field.model] + [Field.reasoning]
    in the user config — effective the next turn, not a hot-swap of the live
    attachment (old [lib/tui/runtime.ml] [save_model_selection] is the reference).
    A locked (unauthenticated) provider's model is shown muted rather than hidden:
    selecting it does not use it — tui-next has no auth flow yet — so {!Login_required}
    lets the shell flash the honest placeholder instead. *)
...
type event =
  ...
  | Select of {
      ...
      (** [↵] on an unlocked model: persist [selector] as the default model and
          [effort] as the default reasoning, then close. Effective the next turn.
      *)
...
(** [update msg t] folds one key. A printable narrows the filter and resets the
    ...
    effort within the highlighted model's supported set; [↵] yields {!Select} for
    an unlocked model or {!Login_required} for a locked one; esc yields {!Close}.
    Every other message is [(t, Stay)]. *)

val view : frame:Mosaic.Ansi.Color.t -> width:int -> rows:int -> t -> _ Mosaic.t
(** [view ~frame ~width ~rows t] renders the panel through {!Panel.view}, [frame]
    tinting the boundary and the [model] chip. The content is the provider-grouped
    model list — muted headers, a blank line above each group after the first, a
    [❯] accent cursor and hover tint on the selection, locked rows muted with
    [log in to use], the current model's trailing [✓] — windowed to keep the
    transcript visible (03-ia open question 1: the list windows rather than
    growing unbounded, [↑ N]/[↓ N more] at the seams, the window sized from
    [rows]), then the highlighted model's effort line ([○◐●◉] intensity glyph,
    level word, [(default)] marker, [← → to adjust], or a muted
