...
module Gguf : sig
  ...
  (** [model ~weights_bytes t] derives guard inputs from [t] and the full file's
      ...
      If both [<arch>.attention.key_length] and [<arch>.attention.value_length]
      are present, their mean is used for the per-head KV dimension and rounded
      up when fractional. The [embedding_length / attention.head_count] fallback
      must divide evenly.
