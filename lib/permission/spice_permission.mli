...
(** Pure permission modelling for agent operations.
    ...
    This library defines inert access facts, permission requests, and pure
    policy evaluation for host-managed operations.

    The usual flow is: construct trusted {!Access.t} facts in host/tool code,
    group them with {!Request.of_accesses} or {!Request.make}, and evaluate with
    {!Policy.decide}. Reviewer answers and their lifetimes belong to sessions.
